...
public class FileNameEncoder {
   ...
   *
   * As of version 1.2 this method will properly handle input containing junk characters (newlines
   * and the like) rather than throwing an error. It does this by pre-parsing the input and
  ...
  static public byte[] decode(char[] data) {
    ...
    for (int ix = 0; ix < data.length; ix++) {
      ...
      if ((value < 0) && (data[ix] != 94)) { // 61 is the '=' symbol (a padding null byte)
  ...
  public static void main(String[] args) {
    if (args.length != 2) {
      System.out.println("Usage: java com.geong.cms.http.FileNameEncoder -e|-d string.");
      System.exit(1);
    }
    if (args[0].equals("-e")) {
      System.out.println(encode(args[1]));
      System.out.println(decode(encode(args[1])));
    } else {
